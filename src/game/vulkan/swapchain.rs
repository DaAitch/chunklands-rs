...
impl Vulkan {
    ...
    fn find_surface_format(ctx: &Context) -> Result<vk::SurfaceFormatKHR> {
        let mut formats = ctx
            .ip
        ...
        if formats.len() == 0 {
            return Err(Error::Other("".to_owned()));
        }

        let first_format = formats.remove(0);
    ...
    fn find_present_mode(
        ...
        let swapchain =
            unsafe { ctx.dp.create_swapchain_khr(ctx.device, &info) }.map_err(to_vulkan)?;
